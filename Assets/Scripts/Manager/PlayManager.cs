...
public class PlayManager : MonoBehaviourPunCallbacks
{
    ...
    public void Spawn()
    {
        if (PhotonNetwork.LocalPlayer.ActorNumber == PhotonNetwork.MasterClient.ActorNumber)
        {
			...
			{
                spawnOnce = true;
                player = PhotonNetwork.Instantiate("Prefabs/Player1", p1Pos.position, new Quaternion(0, 180, 0, 0));
        ...
        else
        {
            ...
            {
                spawnOnce = true;
                player = PhotonNetwork.Instantiate("Prefabs/Player2", p2Pos.position, Quaternion.identity);
                string name = DBManager.nickname;
                string id = DBManager.username;
                int point = DBManager.gp;
    ...
    {
        string sqlConnect = DBManager.sqlConnect;
		...
		{
            string quary = "INSERT INTO Gunfight2Result VALUES(DEFAULT, '" + p1ID + "', '" + p2ID + "', " +
